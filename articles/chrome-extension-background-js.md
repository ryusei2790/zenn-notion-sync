...
微分したい時に１から微分を定義して関数にする必要がなく、関数をbibun();みたいにして()の中に微分したい関数を入力すると、微分した関数が出力されるような感じです。
...
`async/await` を使うと、「この処理が終わるまで待ってから次に進んでね」と書けるようになります。ネット通信が完了してから、ネット通信で取得したデータを使って何かを作ってねって言った感じです。
